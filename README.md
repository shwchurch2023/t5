...
sudo -u hugo zsh -c 'RUN_ID=$(date +%s)-sync /bin/bash /mnt/hugo/github/t5/bin/sync.sh "$RUN_ID" > /mnt/hugo/github/sync.log 2>&1' > /dev/null 2>&1 &; tail -f /mnt/hugo/github/sync.log
...
sudo -u hugo zsh -c 'HUGO_SYNC_INCREMENTAL=1 HUGO_SYNC_DEPLOY_END_STEP=300 RUN_ID=$(date +%s)-sync /bin/bash /mnt/hugo/github/t5/bin/sync.sh "$RUN_ID" > /mnt/hugo/github/sync.log 2>&1' > /dev/null 2>&1 &; tail -f /mnt/hugo/github/sync.log
...
sudo -u hugo zsh -c 'HUGO_SYNC_INCREMENTAL=1 HUGO_SYNC_DEPLOY_END_STEP=290 RUN_ID=$(date +%s)-sync /bin/bash /mnt/hugo/github/t5/bin/sync.sh "$RUN_ID" > /mnt/hugo/github/sync.log 2>&1' > /dev/null 2>&1 &; tail -f /mnt/hugo/github/sync.log
```
